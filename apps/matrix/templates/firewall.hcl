...
FW_ALLOW_IN="{{ env "attr.unique.network.ip-address" }}:tcp:8008,{{ env "attr.unique.network.ip-address" }}:tcp:5432,{{ range nomadService "load-balancer-http" }}{{ .Address }}:tcp:8008{{ end }},{{ range nomadService "mas-http" }}{{ .Address }}:tcp:8008{{ end }}"
...
variable "element_firewall_config" {
  ...
  default     = <<EOH
FW_DNS="{{ with nomadVar "nomad/jobs" }}{{ .dns_server_address }}{{ end }}"
FW_ALLOW_IN="{{ env "attr.unique.network.ip-address" }}:tcp:8080,{{ env "attr.unique.network.ip-address" }}:tcp:8081,{{ range nomadService "load-balancer-http" }}{{ .Address }}:tcp:8080,{{ .Address }}:tcp:8081{{ end }}"
EOH
}
...
variable "mas_firewall_config" {
  ...
  default     = <<EOH
FW_DNS="{{ with nomadVar "nomad/jobs" }}{{ .dns_server_address }}{{ end }}"
FW_ALLOW_IN="{{ env "attr.unique.network.ip-address" }}:tcp:8080,{{ env "attr.unique.network.ip-address" }}:tcp:5432,{{ range nomadService "load-balancer-http" }}{{ .Address }}:tcp:8080{{ end }},{{ range nomadService "matrix-http" }}{{ .Address }}:tcp:8080{{ end }}"
...
variable "rtc_firewall_config" {
  ...
  default     = <<EOH
FW_DNS="{{ with nomadVar "nomad/jobs" }}{{ .dns_server_address }}{{ end }}"
FW_ALLOW_IN="0.0.0.0/0:udp,{{ range nomadService "load-balancer-http" }}{{ .Address }}:tcp:7880,{{ .Address }}:tcp:8080{{ end }}"
